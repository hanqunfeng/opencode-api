...
2. 使用固定大小线程池（`Executors.newFixedThreadPool`，大小通过 `opencode.sse.thread-pool-size` 配置，默认 4）打开到 OpenCode Server 的 SSE 连接。当所有线程被占用时，新 SSE 请求的任务进入队列等待；极端情况下队列满则返回 503 Service Unavailable
